...
router.route('/registration')
	...
	// как происходит регистрация: юзер отправляет свои данные, ему отправляется письмо со ссылкой для подтверждения имейла и
	// создается запись о попытке регистрации в БД (registrationData). После перехода по ссылке из письма (подтверждения имейла)
	// завершается регистрация и создаются записи о юзере в БД (user - данные юзера, userInfo - информация о юзере, отображаемая
	// на форуме, userVisitData - данные о последних просмотрах юзером чатов).
	// Записи о попытках регистрации создаются также для ограничения кол-ва отправляемых писем одному юзеру.
	// Для определения устройства используется библиотека fingerprintjs2.
